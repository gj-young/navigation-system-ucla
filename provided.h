...
struct GeoCoord
{
    ...
    {
        bool sameLat = (latitudeText == other.latitudeText);
        if (!sameLat)
        {
            return (latitudeText < other.latitudeText);
        }
        else
        {
            return (longitudeText < other.longitudeText);
        }
    }
    bool operator > (const GeoCoord& other) const
    {
    ...
    bool operator == (const GeoCoord& other) const
    {
        return ((latitudeText == other.latitudeText) && (longitudeText == other.longitudeText)) ;
    }
};

inline ostream& operator << (ostream& os, const GeoCoord& geo)   // Added overloaded <<
{
    os << geo.latitudeText << " " << geo.longitudeText;
    return os;
}
